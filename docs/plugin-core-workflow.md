...
A2 完成后，**必须阻塞等待用户对技术方案的明确确认，不得自动推进到任何后续阶段**。
...
3. **派发循环**（逐组串行）：
   ...
   - **实装约定**（与 `commands/code.md` 一致）：在 Cursor 中通过 **Task** 工具启动类型为 **`generalPurpose`** 的子代理会话；prompt 中嵌入本组任务清单、技术方案相关章节摘取、`AGENTS.md` 要点、TDD 要求，以及 **`agents/coding-subagent.md`** 规定的必须接收/禁止接收边界与「编码执行结果」返回结构。`coding-subagent` 文件定义行为契约，不表示存在独立的运行时「代理类型名」；由主 Agent 在 Task 调用中落实该契约。
   - 派发子代理执行（子代理在隔离上下文中按 TDD 节奏编码）
   - 子代理完成后验收：检查完成状态、审核假设决策、确认测试通过
   - 验收通过 → 记录结果、更新进度、派发下一组
...
每次 commit 后，立即将变更追加到 `docs/delivery/SV-xxxxx-changelist.md`，包含变更文件、变更类型、功能说明、接口信息、高风险标注。此刻 AI 对变更的理解最准确（刚写完代码），记录的精度最高。编排模式下，各子代理将变更记录随结果返回，由主 Agent 在验收后统一追加。
...
B3 以 **`verification-loop` skill** 为**编排层**（详见 `skills/shared/verification-loop/SKILL.md`），按 **V1 → V2 → V3 → V4 → V5** 顺序执行「结构化验证循环」，不是「先跑一遍测试与性能、再另做对齐」的线性杂糅。支持增量重验（每轮只重验仍为 ❌ 的维度）、断点续验，状态与迭代记录写入 `docs/delivery/SV-xxxxx-verification.md`。
...
安全审查**不占用** V1–V5 中某一维的编号，避免与「先对齐后规范」的固定顺序冲突；其结论作为 **B3 收口** 写入评审与检查点，**仍可能**因 Critical 级安全问题要求修复并重走相关验证维度（由主 Agent 与规则判定）。

#### 3.4 审查与验证产出

`verification-loop` 在**第五步**与 B3 收口阶段约定产出（与 skill 原文一致，路径约定不变）：

1. **验证检查点文件**：`docs/delivery/SV-xxxxx-verification.md`（全过程）
2. **代码评审报告**：`docs/delivery/SV-xxxxx-review-report.md`（可整合多维度与对齐、安全等结论）
...
前置条件：B3 中 `verification-loop` 对 V1–V5 已 **综合 PASS**，且 **3.3 安全审计** 已得到可交付结论（无未解决的 Critical，或已修复并重验相关项）。随后由 `doc-updater` agent 检查所有交付文档的完整性、格式合规性和与代码变更的一致性，确认齐套后进入交付展示。
...
/start SV-34577
  ...
  → B3 结构化验证循环（V1–V5，见 `verification-loop`）→ 安全审计（`security-reviewer`）→ 验证检查点 + 评审/技术参考等交付文档
  → 展示交付物 → 提交 PR
```
