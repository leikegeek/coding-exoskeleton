...
2. **如果已存在**：展示当前画像摘要，同时检查全局用户配置 `~/.exoskeleton/user-config.json` 是否存在（兼容读取 legacy `~/.cursor/coding-exoskeleton/user-config.json`），然后询问用户：
...
- 选择 **仅配置个人作者信息**：直接执行第四步，完成后结束 `/init`，不改写 `AGENTS.md` 和 `.exoskeleton/harness-config.json`
...
|---------|----------|---------------|-------------|
| `cola-java` | Java（基于 COLA 分层）项目 | `skills/backend-common/*` + `skills/cola-java/*` | `rules/backend-common/*` + `rules/cola-java/*` |
...
```text
$env:USERPROFILE\.exoskeleton\user-config.json
```
...
生成规则：
1. 优先读取 `$env:USERPROFILE\.exoskeleton\user-config.json`；若不存在，再兼容读取 `$env:USERPROFILE\.cursor\coding-exoskeleton\user-config.json`。已配置则展示摘要并询问是否更新
...
4. 用户确认后写入 `$env:USERPROFILE\.exoskeleton\user-config.json`；该文件不在业务项目目录内，禁止写入 `AGENTS.md` 或 `.exoskeleton/harness-config.json`
...
将技术栈标识写入 `.exoskeleton/harness-config.json`：
...
> **兼容说明**：如果项目已经存在 legacy `.cursor/harness-config.json`（旧项目遗留），仅更新其中的 `techStack` 字段，保留其他配置；不会新增 `.cursor/` 目录或文件。新项目默认写入 `.exoskeleton/harness-config.json`，不再创建 `.cursor/`。`src/hooks/common.ps1` 的 `Resolve-HarnessDir` 优先读取 `.exoskeleton/`，并兼容回退读取历史 `.cursor/`。
...
> - 个人作者配置：`~/.exoskeleton/user-config.json`（如已配置；legacy `~/.cursor/coding-exoskeleton/user-config.json` 仅作为兼容读取路径，Codex 不写入 `~/.codex/config.toml`）
>
...
- `AGENTS.md` 保存在**业务项目根目录**（Cursor、Claude Code、Codex 共同读取的项目画像入口），不在插件目录内
- 作者等个人配置只写入 `~/.exoskeleton/user-config.json`，可兼容读取 legacy `~/.cursor/coding-exoskeleton/user-config.json`；不得写入业务仓库、`AGENTS.md`、`.exoskeleton/harness-config.json`、技术方案或交付文档
