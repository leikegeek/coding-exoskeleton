...
1. 执行当前平台安装的 `harness-report.ps1` 脚本，获取 JSON 格式的统计数据。脚本位置由平台适配层决定：
   - Cursor：`~/.cursor/plugins/local/coding-exoskeleton/hooks/harness-report.ps1`
   - Claude Code：`.claude/hooks/harness-report.ps1`
   - Codex：`.codex/hooks/harness-report.ps1`
   - 开发仓库内调试：`src/hooks/harness-report.ps1`

```powershell
powershell -ExecutionPolicy Bypass -File "<harness-report.ps1>" -Project "<项目根目录>" -Days 7 -OutputJson
```
