...
如果 `/init` 预检发现业务项目已存在 `AGENTS.md`，个人本地配置仍必须单独检查；项目画像的“跳过/增量/覆盖”不应隐式跳过 `~/.exoskeleton/user-config.json` 的配置入口（legacy `~/.cursor/coding-exoskeleton/user-config.json` 仅兼容读取）。
...
以下信息属于开发者个人配置，只能写入 `~/.exoskeleton/user-config.json`，不得写入业务项目或 `AGENTS.md`：
...
- `.exoskeleton/harness-config.json` 中的 `techStack` 字段更新（新项目默认写入 `.exoskeleton/`；历史项目如已有 `.cursor/harness-config.json`，兼容读取并在原地更新，不创建新的 `.cursor/` 目录）
- `~/.exoskeleton/user-config.json`：可选的全局个人配置（作者名、邮箱等），不进入业务仓库；legacy `~/.cursor/coding-exoskeleton/user-config.json` 仅作为兼容读取路径

## 个人配置隔离规则

- `AGENTS.md` 只记录项目共享画像，不记录开发者个人作者名、邮箱或签名偏好。
- `.exoskeleton/harness-config.json` 只记录项目级 harness 配置，不记录个人作者信息（历史项目如已有 `.cursor/harness-config.json`，兼容读取并在原地更新）。
- 全局个人配置固定写入 `~/.exoskeleton/user-config.json`，位于业务项目之外，天然不会被 git 提交；历史 `~/.cursor/coding-exoskeleton/user-config.json` 可作为 fallback 读取。
