...
.DESCRIPTION
    解析 .exoskeleton/hooks/logs/harness-events.jsonl（优先）或 .cursor/hooks/logs/harness-events.jsonl（fallback），
    生成可读的统计报告。支持双平台（Cursor / Claude Code）的治理事件统计。
...
# 优先读取 .exoskeleton/，fallback .cursor/
$exoskeletonEventsFile = Join-Path $ProjectPath ".exoskeleton\hooks\logs\harness-events.jsonl"
$cursorEventsFile      = Join-Path $ProjectPath ".cursor\hooks\logs\harness-events.jsonl"

if (Test-Path $exoskeletonEventsFile) {
    $eventsFile = $exoskeletonEventsFile
} elseif (Test-Path $cursorEventsFile) {
    $eventsFile = $cursorEventsFile
} else {
    Write-Host "未找到事件日志：" -ForegroundColor Red
    Write-Host "  .exoskeleton\hooks\logs\harness-events.jsonl" -ForegroundColor Yellow
    Write-Host "  .cursor\hooks\logs\harness-events.jsonl" -ForegroundColor Yellow
