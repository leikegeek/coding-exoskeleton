...
if (-not [string]::IsNullOrWhiteSpace($declaredMode)) {
    ...
    Write-HarnessEvent -EventType "mode_change" -HookName "codexUserPromptSubmit" -Detail "mode=$modeValue" -Outcome "allow"
}

if ($isNewTask) {
    Write-HarnessEvent -EventType "new_task" -HookName "codexUserPromptSubmit" -Detail "new task detected" -Outcome "allow"
}

Allow-Hook -Message "codex prompt parsed, pass"

} catch {
    Allow-Hook -Message "codex prompt fallback allow"
}
