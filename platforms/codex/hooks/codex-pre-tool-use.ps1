...
if ($tool -match "^(Bash|PowerShell)$") {
    ...
    if ($matchedDeny) {
    Write-HarnessEvent -EventType "deny" -HookName "codexPreToolUse" -Detail "deny-rule: $($matchedDeny.id) | cmd: $command" -Outcome "deny"
    ...
    if ($matchedAsk) {
        Write-HarnessEvent -EventType "ask" -HookName "codexPreToolUse" -Detail "ask-rule: $($matchedAsk.id) | cmd: $command" -Outcome "ask"
        Ask-Hook -Message "Please confirm risky command: $($matchedAsk.id)"
    }

    Allow-Hook -Message "codex shell pre-tool passed"
}
...
if (-not (Test-PathInWhitelist -Path $pathText -Whitelist $whitelist)) {
    ...
    Write-HarnessEvent -EventType "deny" -HookName "codexPreToolUse" -Detail "path=$pathText, allowed=$allowedList" -Outcome "deny"
    Deny-Hook -Reason "Path not in whitelist: $pathText. Allowed: $allowedList"
}

Allow-Hook -Message "codex write pre-tool passed"

} catch {
    Allow-Hook -Message "codex pre-tool fallback allow"
}
